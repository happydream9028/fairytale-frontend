...
const OffcanvasPlacementSingle = ({ name, ...props }: { name: string }) => {
  const [show, setShow] = useState(false);
...
const OffcanvasPlacement = () => (
  <Card>
    <Card.Header>
      ...
      <h6 className="card-subtitle text-muted">
        There’s no default placement for offcanvas components, so you must set the <code>placement</code> parameter.
      </h6>
    </Card.Header>
    <Card.Body className="text-center">
...
const OffcanvasBackdropSingle = ({ name, ...props }: { name: string }) => {
  const [show, setShow] = useState(false);
  ...
  <Card>
    <Card.Header>
      ...
      <h6 className="card-subtitle text-muted">
        ...
        Use the <code>scroll</code> parameter to toggle <code>&#x3C;body&#x3E;</code> scrolling and{" "}
