...
interface LaneType {
  ...
  onContainerLoaded: (container: any) => void;
}

const Lane = ({ name, children, onContainerLoaded }: LaneType) => {
  const handleContainerLoaded = (container: any) => {
    if (container) {
...
const Task = ({ id, checked, text, avatar }: TaskType) => (
  <Card className="mb-3 bg-light cursor-grab border">
    <Card.Body className="p-3">
      <div className="float-end">
        <Form.Check type="checkbox" id={"exampleCustomCheckbox" + id} defaultChecked={checked ? true : false} />
      </div>
      <p>{text}</p>
      <div className="float-end mt-n1">
        <img src={avatar} width="32" height="32" className="rounded-circle" alt="Avatar" />
      </div>
...
const Tasks = () => {
  const onContainerReady = (container: any) => {
